...
class MyBatteryNode : public rclcpp::Node 
{
public:
    MyBatteryNode() : Node("battery") 
    {
        ...
        battery_timer_=this-> create_wall_timer(std::chrono::milliseconds(100),
                                                std::bind(&MyBatteryNode::checkBatteryState,this));
        RCLCPP_INFO(this->get_logger(),"Battery node has been started");                                                
    }
private:
    void setLed(int led_number, int state){
        threads_.push_back(std::thread(std::bind(&MyBatteryNode::callSetLedService,this,led_number,state)));
    }
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<MyBatteryNode>(); 
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}