...
class MyPanelNode : public rclcpp::Node //  
{
public:
    MyPanelNode() : Node("led_panel_node"),led_states_(3,0) // 
    {
        this->declare_parameter("led_states",std::vector<int64_t>{0,0,0});
        led_states_=this->get_parameter("led_states").as_integer_array();
        
        led_states_publisher_= this->create_publisher<my_robot_interfaces::msg::LedStates>("led_states",10);
        led_states_timer_=this->create_wall_timer(std::chrono::seconds(4),
                                                    std::bind(&MyPanelNode::publishLedStates, this));
        set_led_service_=this->create_service<my_robot_interfaces::srv::SetLed>("set_led",
            std::bind(&MyPanelNode::callbackSetLed,this,std::placeholders::_1,std::placeholders::_2));
...
private:
    ...
    rclcpp::Publisher<my_robot_interfaces::msg::LedStates>::SharedPtr led_states_publisher_;
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<MyPanelNode>(); // 
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}